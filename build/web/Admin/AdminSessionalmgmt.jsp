<%@include file="AdminDashStruct.jsp" %>
            ...
            <div class="container-fluid">
                <div class="row bg-title" style="border-bottom:2px solid black">
                    <div class="col-lg-3 col-md-4 col-sm-4 col-xs-12">
                        <h4 class="page-title" style="color:red;font-size:35px;font-weight:bold;font-family: Gabriola;text-shadow: 5px 0px 4px gray;">Sessional_Management</h4> </div>
                    <div class="col-lg-9 col-sm-8 col-md-8 col-xs-12">
                        
                        <button type="submit" style="background: red;height: 40px;width: 150px;border-radius: 10px;margin-left: 400px;box-shadow: 5px 0px 4px gray;"><a href="AdminAddsessional.jsp" style="color: white;text-decoration: none;font-size: 15px;font-weight: bold;">Add </a></button>
                         <button type="submit" style="background: red;height: 40px;float: left; width: 150px;border-radius: 10px;margin-left: 580px;box-shadow: 5px 0px 4px gray;"><a href="AdminUpdateSessional.jsp" style="color: white;text-decoration: none;font-size: 15px;font-weight: bold;">Update/Delete</a></button>
                    </div>
                    <!-- /.col-lg-12 -->
                </div>
                   ...
                   <table class="table table-responsive" style="background:white">
                    ...
                    <%
                        ...
                        if(request.getParameter("opr1")!= null){
                          ...
                          db.pstmt = db.con.prepareStatement("select * from sessional where branch=? and session=? and subject=? order by date");
                         db.pstmt.setString(1,branch);
                         db.pstmt.setString(2,stype);                         
                         db.pstmt.setString(3,subject); 
                          
                          db.rst = db.pstmt.executeQuery();
                            out.println("<tr><th>RollNo</th><th>Year</th><th>Semester</th><th>Subject</th><th>Sessional 1</th><th>Sessional 2</th> <th>Sessional 3</th><th>Date</th><th>Delete</th></tr>");
                          while(db.rst.next()){
                          
                            out.println("<tr><td>"+db.rst.getString(4)+"</td><td>"+db.rst.getString(2)+"</td><td>"+db.rst.getString(3)+"</td>"
                                    + "<td>"+db.rst.getString(5)+"</td><td>"+db.rst.getString(6)+"</td><td>"+db.rst.getString(7)+"</td><td>"+db.rst.getString(8)+"</td><td>"+db.rst.getString(9)+"</td><td>"
                                    + "<button><a href=Admindeletesessional.jsp?sub="+db.rst.getString(5)+",rno="+db.rst.getString(6)+",date="+db.rst.getString(9)+" >Delete</a> </button></td></tr>");
                          } 
                      }
                        ...
                        if(request.getParameter("opr2")!= null){
                          ...
                          DB.DBConnection db = new DB.DBConnection();
                          db.pstmt = db.con.prepareStatement("select * from sessional where session=? and rollno=? order by date");
                          db.pstmt.setString(1,stype);
                          db.pstmt.setString(2,rno);
                          db.rst = db.pstmt.executeQuery();
                           out.println("<tr><th>Branch</th><th>Year</th><th>Semester</th><th>Roll No</th><th>Subject</th><th>Sessional 1</th><th>Sessional 2</th><th>Sessional 3</th> <th>Date</th><th>Delete</th></tr>");
                           while(db.rst.next()){
                           
                            out.println("<tr><td>"+db.rst.getString(1)+"</td><td>"+db.rst.getString(2)+"</td><td>"+db.rst.getString(3)+"</td>"
                                    + "</td><td>"+db.rst.getString(4)+"</td><td>"+db.rst.getString(5)+"</td><td>"+db.rst.getString(6)+"</td><td>"+db.rst.getString(7)+"</td><td>"+db.rst.getString(8)+"</td><td>"+db.rst.getString(9)+"</td><td>"
                                    + "<button><a href=Admindeletesessional.jsp?desc="+db.rst.getString(4)+",date="+db.rst.getString(8)+" >Delete</a> </button></td></tr>");
                          }
                        }  
                      }catch(Exception e){
                          e.printStackTrace();
                            ...
                            out.println("<form action=# >"
                                    +"<select name=stype class=ip1 >"
                         +"   <option>Select Sessional</option>  <option>2023-24</option><option>2024-25</option>"
                       +" </select>"
                       +" <select name=branch class=ip1>"
 ...
 +" <option>UG Diploma in German</option>"
                      ...
                      +"      <select name=subject class=ip1>"
                       ...
                       +"         <option>DBMS</option>"
                       +"     </select>"
                      +"      <input type=text name=rno class=ip1  placeholder=Enter Roll No... >"
                   +"     <input type=submit value='Search By Subject' name=opr1 class=ip2>"
                     +"   <input type=submit value='Search By Rno' name=opr2 class=ip2 >"
                     +"   </form>" );
                        }
