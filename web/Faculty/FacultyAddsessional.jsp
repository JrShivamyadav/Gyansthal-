      ...
      <option>UG Diploma in German</option>
					...
					<center><button  type="submit" style="height:40px;width:150px;border-style:none;background:red;color:white;font-size:18px;text-align:center">Add</button></center>
                                  ...
                                  <%
                                   ...
                                   if(request.getParameter("rno")!= null){
                                      ...
                                      if(request.getParameter("stype").equals("sessional1")) {
                                        ...
                                        db.pstmt = db.con.prepareStatement("insert into sessional(branch,session,sem,rollnumber,subject,sessional1,date) values(?,?,?,?,?,?,curdate())");
                                  ...
                                  %>
                                        ...
                                        DB.DBConnection db = new DB.DBConnection();
                                        db.pstmt = db.con.prepareStatement("update sessional set sessional2=?,date=curdate() where rollnumber=? and sem=? and branch=? and subject=? and session=? ");
                                        ...
                                        DB.DBConnection db = new DB.DBConnection();
                                        db.pstmt = db.con.prepareStatement("update sessional set sessional3=?,date=curdate() where rollnumber=? and sem=? and branch=? and subject=? and session=? ");
                                        db.pstmt.setString(1,st);
                                        db.pstmt.setString(2, rno);
                                        db.pstmt.setString(3, sem);
                                        db.pstmt.setString(4, branch);
